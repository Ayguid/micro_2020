                            ...
                            @foreach ($data['countries'] as $country)
                              ...
                              <th scope="row">{{$country->country_desc}}</th>
                                @foreach ($data['job_titles'] as $title)
                                  ...
                                  <td><input disabled {{$data['user']->hasTitle($country->id, $title->id)?'checked':''}}  type="checkbox" class="m-2 form-control @error('job_title') is-invalid @enderror" name="job_title[]" value='{{$country->id.','.$title->id}}' ></td>
                                @endforeach
                              </tr>
                              @endforeach
                        </table>
